...
static int up_setup(struct uart_dev_s *dev)
{
  ...
  up_serialout(priv, UART_IER,
               UART_IER_IRxE | UART_IER_ILE);
...
static int up_attach(struct uart_dev_s *dev)
{
  ...
  up_serialout(priv, UART_IER, UART_IER_IRxE | UART_IER_ITxE);
  ...
  for (passes = 0; passes < 256; passes++)
    {
      ...
      status = up_serialin(priv, UART_IIR);
...
static void up_rxint(struct uart_dev_s *dev, bool enable)
{
  ...
  up_serialout(priv, UART_IER, priv->im);
...
static bool up_rxavailable(struct uart_dev_s *dev)
{
  ...
  uint32_t rxdata = up_serialin(priv, UART_RBR);

  g_rxdata = rxdata & 0xff;

  dr = up_serialin(priv, UART_LSR) & UART_LSR_DR;
...
{
  struct up_dev_s *priv = (struct up_dev_s *)dev->priv;
  up_serialout(priv, UART_THR, (uint32_t)ch);
}
...
static bool up_txready(struct uart_dev_s *dev)
{
  ...
  return !!(up_serialin(priv, UART_LSR) & UART_LSR_THRE);
}
...
/****************************************************************************
 * Name: la64_earlyserialinit, la64_serialinit, and up_putc
 *
 * Description:
...
{
  /* UART0 initiaze */
  writeb(0x80, UART0_BASE + 0x3);

  writeb(UART_DIV_HI, UART0_BASE + 0x1);
  writeb(UART_DIV_LO, UART0_BASE + 0x0);

  writeb(0x3, UART0_BASE + 0x3);
